...
st.caption("Interface web equivalente ao notebook do Colab, com os mesmos tipos, campos e lógicas.")
...
with col1:
    nome_emp = st.text_input("Empreendimento:", "")
    endereco_emp = st.text_input("Endereço:", "")
    bairro_emp = st.text_input("Bairro:", "")
with col2:
    cidade_emp = st.text_input("Cidade (Cidade/UF):", "")
    area_total_emp = st.text_input("Área total da gleba (m²):", "")
    matricula_emp = st.text_input("Matrícula(s):", "")
...
if tipo in ("Memorial Condomínio", "Memorial Loteamento"):
    ...
    with c1:
        ...
        perimetro_emp = st.text_input("Perímetro da gleba (m):", "")
    with c2:
        ...
        ane_drop = st.selectbox("Possui faixa não edificante (ANE)?", ["Não", "Sim"])
        if ane_drop == "Sim":
            ane_largura = st.text_input("Largura da faixa (m):", "3,00")
...
if tipo in (
    ...
    st.markdown("### Arquivos (HTML/TXT)")
    st.write(
        "- Quadras/lotes em HTML/TXT (Parcel Report)\n"
        "- CivilReport para áreas gerais (APP, verde, vias, etc.)"
    )
    up_files = st.file_uploader(
        "Anexar arquivos do Civil 3D",
        type=["html", "htm", "txt"],
...
with b1:
    if st.button("Gerar DOCX"):
        try:
            ...
            st.success(f"DOCX gerado: {filename}")
            st.download_button(
                "Baixar DOCX",
                data=doc_bytes,
                file_name=filename,
